...
//                 <TableCell>
//                   {parseFloat(log.work_quantity).toFixed(2).replace(/\.00$/, '')} {log.payment_type}
//                 </TableCell>
          ...
          <TableBody>
            {workLogs.map((log) => (
              <TableRow key={log.id_work_logs}>
                ...
                <TableCell>
                  {log.description}
                  {/* אם העבודה היא עבודה מיוחדת, נציג גם את "תיקונים" */}
                  {log.is_special_work && log.is_special_work === true && (
                    <Typography variant="body2" color="secondary" sx={{ display: 'inline', ml: 2 }}>
                      תיקונים
                    </Typography>
                  )}
                </TableCell>
