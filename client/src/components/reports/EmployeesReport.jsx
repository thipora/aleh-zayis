...
const EmployeesReport = () => {
  ...
  const filteredSummary = summary
    .filter(emp => !selectedRole || emp.role_name === selectedRole)
    .filter(emp => selectedCurrency === "all" || emp.currency === selectedCurrency);
        ...
        <Box display="flex" justifyContent="space-between" alignItems="center" mb={2}>
          ...
          <Box display="flex" gap={1}>
            ...
            <FormControl sx={{ minWidth: 140 }}>
              <InputLabel>{t("employeesReport.filterCurrency")}</InputLabel>
              <Select
                value={selectedCurrency}
                onChange={(e) => setSelectedCurrency(e.target.value)}
              >
                <MenuItem value="all">{t("employeesReport.allCurrencies")}</MenuItem>
                <MenuItem value="ILS">{t("employeesReport.ILS")}</MenuItem>
                <MenuItem value="USD">{t("employeesReport.USD")}</MenuItem>
              </Select>
            </FormControl>
            ...
            <TableBody>
              {filteredSummary.map((emp, i) => (
                ...
                >
                  ...
                  <TableCell align="center" sx={{ direction: i18n.language === "he" ? "rtl" : "ltr" }}>
                    {`${formatCurrency(emp.currency)} ${emp.total}`}
                  </TableCell>
                </TableRow>
              ))}
